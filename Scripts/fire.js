...
script.addFloatParameter("brightness", "", 1,0,1);
...
function recalculateTemperature(heat, params) {
...
function updateColors(colours, id, resolution, time, params, prop) {

  var pid = prop.general.globalID.get();

  // Support N props at the same time
  while (global_heat.length < pid + 1) {
    global_heat.push([]);
    lastTime.push(0);
  }

  // Unknown prop size
  while (global_heat[pid].length < resolution) {
    var pixel = {
      r: 0,
      g: 0,
      b: 0,
    ...
    global_heat[pid].push(pixel);
  }

  if (time != lastTime[pid]) {
    lastTime[pid] = time;
    recalculateTemperature(global_heat[pid], params);
  }

  for (var i = 0; i < resolution; i++) {
    colours.set(
      i,
      global_heat[pid][i].r * params.brightness,
      global_heat[pid][i].g  * params.brightness,
      global_heat[pid][i].b * params.brightness,
      1
    );
  }
}
