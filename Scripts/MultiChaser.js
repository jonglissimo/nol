...
function updateColors(colours, id, resolution, time, params, prop)//, num, speed, size, color, color2, pingPong, double)
{
	...
	for(var i=0;i<resolution;i++)
	{
		...
		colours.set(inv?resolution-1-i:i,
			Math.min(params.color[0]*val1+params.color2[0]*val2,1), 
			Math.min(params.color[1]*val1+params.color2[1]*val2,1),
			Math.min(params.color[2]*val1+params.color2[2]*val2,1)
			);
	}
